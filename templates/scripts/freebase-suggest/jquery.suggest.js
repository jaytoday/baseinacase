...
//                    self.$input.focus();
        ...
        /*
         ...
         if (top != scrollTop) {
         this.$list.animate({scrollTop: top}, "slow", function() {
         $first.trigger(ns("mouseover", self.name));
         });
         }
         }
         */
      ...
      position: function() {
        ...
        var left = true;
        if (pos.left > (scroll_left + window_width/2)) {
          left = false;
        }
        if (!left) {
...
//          this.$input.focus();
    ...
    $.extend($.suggest, {
               defaults: {
                 ...
                 // css prefix that is prepended to the top level container classes (fbs-pane and fbs-flyout-pane)
    ...
    $.suggest("suggest", {
                ...
                request: function(val, next) {
                  ...
                  var ac_param = {};
                  $.each(["type", "strict", "mql_filter", "as_of_time"], function(i,n) {
                           ac_param[n] = self.options[n];
                         });

                  // legcy options.ac_param
                  ...
                  $.ajax({
                           ...
                           "success": function(data, status) {
                             self.response(data, next ? next : -1);
                           },
                ...
                response_hook: function(response_data, next) {
                  if (this.$flyoutpane) {
                    this.$flyoutpane.hide();
                  }
                  if (next > 0) {
                ...
                show_hook: function(response_data, next, $first) {
                  var opts = this.options,
                  self = this;
                  ...
                  // more
                  if (response_data.result && response_data.result.length && "next" in response_data) {
                    if (!$more.length) {
                      ...
                      $(".fbs-more-link", $more)
                        .bind("click.suggest", function(e) {
                                ...
                                $m.data("suggest").more($m.data("next.suggest"));
                              });
                      this.$list.after($more);
                    }
                    $more.data("suggest", self);
                    $more.data("next.suggest", response_data.next);
                  }
                  else {
                    $more.remove();
                  }
                ...
                more: function(next) {
                  if (next) {
                    ...
                    this.request($.trim(this.$input.val()), next);
                  }
                  return false;
                },
    ...
    $.extend($.suggest.suggest, {
               defaults: {
                 ...
                 service_path: "/api/service/suggest",
